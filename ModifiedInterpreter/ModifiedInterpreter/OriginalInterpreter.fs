...
let private freshName baseName used =
    let baseName = if String.IsNullOrWhiteSpace baseName then "v" else baseName
...
let rec substitute term variable replacement =
    ...
    | App(left, right) -> App(substitute left variable replacement, substitute right variable replacement)

    | Lam(parameter, body) when parameter = variable -> Lam(parameter, body)
    ...
    | Lam(parameter, body) ->
        ...
        if
            not (Set.contains parameter replacementFreeVars)
            || not (Set.contains variable bodyFreeVars)
        then
...
let rec reduceOnce term =
    match term with
    | App(Lam(parameter, body), argument) -> Some(substitute body parameter argument)
...
/// </summary>
let interpret maxSteps term = normalize maxSteps term
    ...
    let rec normalizeInner environment usedVars counter term =
        ...
        | Lam(parameter, body) ->
            ...
            let normalizedBody, finalUsedVars, finalCounter =
                normalizeInner (Map.add parameter newParameter environment) newUsedVars (counter + 1) body

            Lam(newParameter, normalizedBody), finalUsedVars, finalCounter

    let normalizedTerm, _, _ = normalizeInner Map.empty initialUsedVars 0 term
...
let rec private format precedence term =
    ...
    | Lam _ ->
        ...
        let text = "\\" + String.concat " " parameters + "." + format 0 body
